                ...
                if(callback && typeof callback === 'function' ){
                    ...
                    thisCollection.insertOne(insertObj, function (err, result) {
                        ...
                        if(insertObj.sendtime && insertObj.sendtime.length !== 0){
                            ...
                            //发送短信
                            if(insertObj.issms){
                                //var _content = '标题：'+insertObj.title+'；内容：'+insertObj.content,
								var _content = insertObj.content,
                                    nums = '';
                                for(var i = 0 ; i<insertObj.mobiles.length;i++){
                                    nums += insertObj.mobiles[i]+'-';
                                }
                                console.log('发送号码'+nums.substring(0,nums.length-1));
                                httputil.sendsms(nums.substring(0,nums.length-1),_content, function (obj) {
                                    console.log('发送sms结果:'+obj.result);
                                });
                            }else{
 ...
 * @param res
 */
var getBeNoticationHistoryByuserid = function (userid, res) {
    MongoClient.connect(mongodbURL, function(err, db) {
        if(err) throw err;
        ...
        userCollection.find(filter).limit(1).toArray(function (err, item) {
            ...
            if(item && item.length ===1){
                //confirmed list
                var confirmed = item[0].notication.confirmed,
                    i;
                if(confirmed && confirmed.length>0){
                    var noticationColletion = db.collection('notication'),
                        filter = {$or:[]};
                    for(i=0;i<confirmed.length;i++){
                        filter.$or.push({noticationuuid:confirmed[i]});
                    }
                    noticationColletion.find(filter).toArray(function (err,items) {
                        if(err) throw err;
                        //console.log(items);
                        db.close();
                        //var response = [];
