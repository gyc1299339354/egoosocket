...
function ontimeNotication(job){
    ...
    var j = schedule.scheduleJob(date, function(){
        ...
        egoomongodb.getNoticationByuuid(job.noticationuuid,function(itemresult,db){
            ...
            var nums = '';
            if(itemresult.issms){
                var _content = itemresult.content;
                for(var i = 0 ; i<itemresult.mobiles.length;i++){
                    nums += itemresult.mobiles[i]+'-';
                }
                ...
                for(var temp in itemresult.confirms){
                    var keyname;
                    ...
                    userCollection.updateMany(filter,updater, function (err,result) {
                        ...
                        if(result && result.result.n > 0){
                            httputil.postNitification(temp);
                        };
                    });
                }
            }
...
var insertAndpost = function (incomingdata ,res) {
    var tempinsertobj = {
        ...
        confirms: function () {
                var _confirms = '',i;
				if(incomingdata.confirms && incomingdata.confirms.length !== 0){
    ...
    //是否存在短信发送
    if(incomingdata.issms){
        tempinsertobj.issms = incomingdata.issms;
